...
TEST(colaPriorATest, encolarMayor) {
    ...
    q.encolar(per3);
    ASSERT_FALSE(q.vacia());
    //ASSERT_TRUE(q.proximo() == 3 && q.proximo().getGastoPersona()==300);
    ASSERT_TRUE(q.proximo() == 3);
}
...
TEST(colaPriorATest, tres_compras) {
    ...
    EXPECT_EQ(q.proximo(), 2);
}

TEST(colaPriorATest, vender_2_veces_dist_personas) {
    colaPriorA<Nat,Persona> cola;
